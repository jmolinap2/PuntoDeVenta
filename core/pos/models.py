...
class Sale(models.Model):
    ...
    dscto = models.DecimalField(default=0.00, max_digits=9, decimal_places=2)
    total_dscto = models.DecimalField(default=0.00, max_digits=9, decimal_places=2)
    ...
    def delete(self, using=None, keep_parents=False):
        for detail in self.saleproduct_set.filter(product__is_service=False):
    ...
    def calculate_invoice(self):
        ...
        self.total_dscto = self.subtotal * (float(self.dscto) / 100)
        self.total = float(self.subtotal) + float(self.total_iva) - float(self.total_dscto)
        self.save()
